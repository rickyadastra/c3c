...
static void init_asm_x86(PlatformTarget* target)
{
	...
	reg_instr(target, "pause", NULL);
	reg_instr(target, "in", "w:r8/r16/r32, r16/imm8"); // Actually ensure reg_al_ax and dx
	reg_instr(target, "out", "r16/imm8, r8/r16/r32"); // Actually ensure reg_al_ax and dx
	...
	if (target->arch == ARCH_TYPE_X86)
	{
		...
		reg_register_list(target, x86_ctrl_regs, 4, ASM_REG_INT, ARG_BITS_32, X86_CR0);
	}
	else
	{
		...
		reg_register_list(target, x86_ctrl_regs, 4, ASM_REG_INT, ARG_BITS_64, X86_CR0);
		reg_register(target, "$cr8", ASM_REG_INT, ARG_BITS_64, X86_CR8);
	}
}
